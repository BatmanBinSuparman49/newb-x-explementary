...
void main() {
 ...
 if(detecttexture){
  ...
  float shadowFactor = 1.0 - 0.05 * shadow;
  shadowFactor = max(shadowFactor, 0.3); // Prevents shadows from going darker than 85% brightness
  diffuse.rgb *= shadowFactor;

  // side block shadows
  float sideshadow = smoothstep(0.64, 0.62, v_color1.g);
  diffuse.rgb *= 1.0-0.09*sideshadow;                     //increase 0.38 = darker shadow
