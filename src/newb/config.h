#ifndef NL_CONFIG_H
#define NL_CONFIG_H

/* Color correction */
#define NL_TONEMAP_TYPE 9              // 1:Exponential, 2:Reinhard, 3:Extended Reinhard, 4:ACES 5:John Habel's filmic (used in BSBE) 
#define NL_GAMMA 1.1                  // 0.3 low ~ 2.0 high
//#define NL_EXPOSURE 1.3              // [toggle] 0.5 dark ~ 3.0 bright
#define NL_SATURATION 0.9            // [toggle] 0.0 grayscale ~ 4.0 super saturated
//#define NL_TINT                      // [toggle] enable light/dark tone tinting
#define NL_TINT_LOW  vec3(0.3, 0.5, 1.2) // color tint for dark tone
#define NL_TINT_HIGH vec3(1.3, 0.6, 0.2)  // color tint for light tone

/* Terrain lighting */
#define NL_SUN_INTENSITY 3.8   // 0.5 weak ~ 5.0 bright
#define NL_TORCH_INTENSITY 1.0  // 0.5 weak ~ 3.0 bright
#define NL_NIGHT_BRIGHTNESS 0.0 // 0.0 dark ~ 2.0 bright
#define NL_CAVE_BRIGHTNESS 1.8  // 0.0 dark ~ 2.0 bright
#define NL_SHADOW_INTENSITY 0.2 // 0.0 no shadow ~ 1.0 strong shadow
//#define NL_BLINKING_TORCH     // [toggle] flickering light
//#define NL_CLOUD_SHADOW       // [toggle] cloud shadow (simple clouds only)

/* Sun/moon light color on terrain */
#define NL_MORNING_SUN_COL vec3(0.98, 0.84, 0.73)
#define NL_NOON_SUN_COL    vec3(0.98, 0.90, 0.73)
#define NL_NIGHT_SUN_COL   vec3(0.4039, 0.3804, 0.9804)

/* Ambient light on terrain (light that is added everywhere) */
#define NL_NETHER_AMBIENT vec3(3.0, 2.18, 1.73) * 0.8 //previously vec3(0.5, 0.25, 0.2)
#define NL_END_AMBIENT    vec3(0.23,0.0,0.9)

/* Torch colors */
#define NL_OVERWORLD_TORCH_COL  vec3(1.00, 0.5, 0.3)
#define NL_UNDERWATER_TORCH_COL vec3(2.30,2.30,2.30)
#define NL_NETHER_TORCH_COL     vec3(1.0,0.52,0.18)
#define NL_END_TORCH_COL        vec3(0.55, 0.24, 0.89)

/* Fog */
#define NL_FOG 0.8                // [toggle] 0.1 subtle ~ 1.0 blend with sky completely
#define NL_MIST_DENSITY 1.0      // 0.0 no mist ~ 1.0 misty
#define NL_RAIN_MIST_OPACITY 0.5 // [toggle] 0.04 very subtle ~ 0.5 thick rain mist blow
// #define NL_CLOUDY_FOG 0.8         // [toggle] 0.0 subtle - 0.8 dense fog clouds

/* Sky colors - zenith=top, horizon=bottom */
#define NL_DAY_ZENITH_COL    vec3(0.0, 0.12, 0.52) //previously vec3(0.1333, 0.5176, 0.9020)
#define NL_DAY_HORIZON_COL   vec3(0.573, 0.612, 0.651) //previously vec3(0.752, 0.902, 0.980)
#define NL_NIGHT_ZENITH_COL  vec3(0.0, 0.03, 0.065)
#define NL_NIGHT_HORIZON_COL vec3(0.05, 0.28, 0.5) 
#define NL_RAIN_ZENITH_COL   vec3(0.239, 0.231, 0.231)
#define NL_RAIN_HORIZON_COL  vec3(0.451, 0.447, 0.447)

// Colores para las púas (similar al efecto End del Complementary Shader)
#define PUA_COLOR_1 vec3(0.9, 0.4, 2.0)  // Color púrpura brillante
#define PUA_COLOR_2 vec3(1.7, 0.8, 2.5)  // Color azul claro
#define NL_END_ZENITH_COL    vec3(0.0, 0.0075, 0.001) // Color zenit (negro)
#define NL_END_HORIZON_COL   vec3(0.5, 0.4, 1.5) // Color horizonte (rojo oscuro)

// #define NL_END_SKY_TYPE 2   // 1:nr endsky1, 2: nr endsky2, 3:nxl default
//#define NL_END_ZENITH_COL    vec3(0.02,0.0,0.0)
//#define NL_END_HORIZON_COL   vec3(0.48,0.29,0.87)

#define NL_DAWN_ZENITH_COL   vec3(0.2078, 0.1451, 0.3333) 
#define NL_DAWN_HORIZON_COL  vec3(2.5, 0.5804, 0.0096) //previously vec3(1.0, 0.2353, 0.0)
#define NL_DAWN_EDGE_COL     vec3(2.5, 0.5804, 0.0096) //previously vec3(1.0, 0.1647, 0.0)

/* Rainbow */
//#define NL_RAINBOW         // [toggle] enable rainbow in sky
#define NL_RAINBOW_CLEAR 0.5 // 0.3 subtle ~ 1.7 bright during clear
#define NL_RAINBOW_RAIN 1.0  // 0.5 subtle ~ 2.0 bright during rain

/* Ore glow intensity */
#define NL_GLOW_TEX 3.0           // 0.4 weak ~ 8.0 bright
#define NL_GLOW_SHIMMER 0.8       // [toggle] 0.1 subtle ~ 1.0 100% shimmer
#define NL_GLOW_SHIMMER_SPEED 1.0 // 0.5 slow - 2.0 fast
#define NL_GLOW_LEAK 0.4        // [toggle] 0.08 subtle ~ 1.0 100% brightness of NL_GLOW_TEX

/* Waving */
#define NL_PLANTS_WAVE 0.14    // [toggle] 0.02 gentle ~ 0.4 violent
#define NL_LANTERN_WAVE 0.14   // [toggle] 0.05 subtle ~ 0.4 large swing
#define NL_WAVE_SPEED 1.6      // 0.5 slow wave ~ 5.0 very fast wave
#define NL_EXTRA_PLANTS_WAVE // [toggle] !dont use! wave using texture coords (1.21.0 vanilla 2048x1024)

/* Water */
#define NL_WATER_TRANSPARENCY 0.7 // 0.0 transparent ~ 1.0 normal
#define NL_WATER_BUMP 0.2        // 0.001 plain ~ 0.2 bumpy water
#define NL_WATER_WAVE_SPEED  0.6  // 0.2 calm ~ 2.0 turbulent
#define NL_WATER_TEX_OPACITY 0.0  // 0.0 plain water ~ 1.0 vanilla water texture
#define NL_WATER_WAVE             // [toggle] wave effect
//#define NL_WATER_CLOUD_REFLECTION // [toggle] simple clouds/aurora reflection
//#define NL_WATER_REFL_MASK      // [toggle] fake water reflection mask
#define NL_WATER_TINT vec3(1.3, 1.55, 1.45)

/* Underwater */
#define NL_UNDERWATER_BRIGHTNESS 0.4         // 0.0 dark ~ 3.0 bright
#define NL_CAUSTIC_INTENSITY     5.0            // 0.5 weak ~ 5.0 bright
#define NL_UNDERWATER_WAVE       0.14               // [toggle] 0.02 subtle ~ 0.6 trippy
#define NL_UNDERWATER_STREAKS    2.0            // [toggle] 0.8 subtle - 2.0 bright streaks from top
#define NL_UNDERWATER_TINT vec3(0.9, 1.3, 0.9) // fog tint color when underwater

/* Cloud type */
#define NL_CLOUD_TYPE 3 // 0:vanilla, 1:soft, 2:rounded, 3:realistic, 4:PC killer

/* Vanilla cloud settings - make sure to remove clouds.png when using this */
#define NL_CLOUD0_THICKNESS 2.1      // 0.5 slim ~ 8.0 fat
#define NL_CLOUD0_RAIN_THICKNESS 4.0 // 0.5 slim ~ 8.0 fat
#define NL_CLOUD0_OPACITY 0.9        // 0.0 invisible ~ 1.0 opaque
#define NL_CLOUD0_MULTILAYER         // [toggle] extra cloud layer

/* Soft cloud settings */
#define NL_CLOUD1_SCALE vec2(0.016, 0.022) // 0.003 large ~ 0.2 tiny
#define NL_CLOUD1_DEPTH 1.3                // 0.0 no bump ~ 10.0 large bumps
#define NL_CLOUD1_SPEED 0.04               // 0.0 static ~ 0.4 fast moving
#define NL_CLOUD1_DENSITY 0.54             // 0.1 less clouds ~ 0.8 more clouds
#define NL_CLOUD1_OPACITY 0.9              // 0.0 invisible ~ 1.0 opaque

/* Rounded cloud settings */
#define NL_CLOUD2_THICKNESS 2.45      // 0.5 slim ~ 5.0 fat
#define NL_CLOUD2_RAIN_THICKNESS 2.45 // 0.5 slim ~ 5.0 fat
#define NL_CLOUD2_STEPS 16            // 3 low quality ~ 16 high quality
#define NL_CLOUD2_SCALE vec2(0.04, 0.04)        // 0.003 large ~ 0.3 tiny
#define NL_CLOUD2_SHAPE vec2(0.68, 0.68)          // 0.0 round ~ 1.0 box
#define NL_CLOUD2_DENSITY 70.0       // 1.0 blurry ~ 100.0 sharp
#define NL_CLOUD2_VELOCITY 0.8        // 0.0 static ~ 4.0 very fast
//#define NL_CLOUD2_LAYER2                      // [toggle] extra cloud layer
#define NL_CLOUD2_LAYER2_OFFSET 143.0           // 30.0 near ~ 300.0 very high
#define NL_CLOUD2_LAYER2_THICKNESS 2.5          // 0.7 slim ~ 5.0 fat
#define NL_CLOUD2_LAYER2_RAIN_THICKNESS 3.0     // 0.7 slim ~ 5.0 fat
#define NL_CLOUD2_LAYER2_STEPS 3                // 3 low quality ~ 16 high quality
#define NL_CLOUD2_LAYER2_SCALE vec2(0.03, 0.03) // 0.003 large ~ 0.3 tiny
#define NL_CLOUD2_LAYER2_SHAPE vec2(0.5, 0.4)   // 0.0 round ~ 1.0 box vec2(horizontal shape, vertical shape)
#define NL_CLOUD2_LAYER2_DENSITY 25.0           // 1.0 blurry ~ 100.0 sharp
#define NL_CLOUD2_LAYER2_VELOCITY 0.8           // 0.0 static ~ 4.0 very fast

/* Realistic cloud settings */
#define NL_CLOUD3_SCALE vec2(0.05, 0.05) // 0.003 large ~ 1.0 tiny
#define NL_CLOUD3_SPEED 0.01            // 0.0 static ~ 4.0 fast moving
#define NL_CLOUD3_SHADOW 0.59             // 0.1 subtle ~ 1.0 dark
#define NL_CLOUD3_SHADOW_OFFSET 0.79      // 0.05 minimal ~ 1.0 large


/* Aurora settings */
// #define NL_AURORA 3.2           // [toggle] 0.4 dim ~ 4.0 very bright
#define NL_AURORA_VELOCITY 0.08 // 0.0 static ~ 0.3 very fast
#define NL_AURORA_SCALE 0.08    // 0.002 large ~ 0.4 tiny
#define NL_AURORA_WIDTH 0.28    // 0.04 thin line ~ 0.4 thick lines
#define NL_AURORA_COL1 vec3(0.1,1.0,0.0)
#define NL_AURORA_COL2 vec3(0.1,0.0,1.0)

/* Lynx Aurora*/
#define LYNX_AURORA                             // [toggle] LYNX_AURORA :gigachad:
// #define END_LYNX_AURORA

/* Falling Stars by i11212 */
#define FALLING_STARS                             // [toggle]
#define NL_FALLING_STARS_VELOCITY    10.0    
#define NL_FALLING_STARS_DENSITY     0.004        // 0.001 rare ~ 0.05 many
#define NL_FALLING_STARS_SCALE       28.0          // greater value = smaller stars

/* Starfield */
#define STARFIELD
#define NUM_LAYERS 2.0
#define TAU 6.28318
#define PI 3.141592
#define VELOCITY 0.001 //modified value to increse or decrease speed, negative value travel backwards
#define StarGlow 0.138
#define StarSize 2.3
#define CanvasView 6.2 

/* Shooting star */
#define NL_SHOOTING_STAR 1.0        // [toggle] 0.2 dim ~ 1.0 bright
#define NL_SHOOTING_STAR_PERIOD 0.9 // 0.4 fast ~ 12.0 slow (12 secs)
#define NL_SHOOTING_STAR_DELAY 16.0 // 0.0 no delay ~ 100.0 long time (100 secs)
#define STAR_SCALE             0.45    // 1.0 default ~ >1.0 bigger

/* Galaxy */
// #define NL_END_GALAXY_STARS 4.0           // [toggle] 0.1 dim - 4.0 bright
#define NL_END_GALAXY_VIBRANCE 1.0
#define NL_END_GALAXY_SPEED  0.01
#define NL_END_GALAXY_DAY_VISIBILITY 0.0

// Overworld Galaxy
#define NL_GALAXY_STARS     4.0           // [toggle]
#define NL_GALAXY_VIBRANCE 0.8          // 0.0 white - 1.0 colorful
#define NL_GALAXY_SPEED 0.01            // 0.01 slow motion - 0.2 fast motion
#define NL_GALAXY_DAY_VISIBILITY 0.0    // 0.0 invisible - 1.0 visible

/* Chunk loading slide in animation */
#define NL_CHUNK_LOAD_ANIM 100.0 // [toggle] -600.0 fall from top ~ 600.0 rise from bottom

/* Sun/Moon */
#define NL_SUNMOON_ANGLE 90.0 // [toggle] 0.0 no tilt ~ 90.0 tilt of 90 degrees
#define NL_SUN_SIZE 0.2     // 0.3 tiny ~ 4.0 massive
#define NL_MOON_SIZE 0.45     //separate moon size scale
/* Fake godrays during sunrise/sunset */
#define NL_GODRAY 0.8 // [toggle] 0.1 subtle ~ 0.8 strong

/* Sky reflection */
#define NL_GROUND_REFL 0.7       // [toggle] 0.2 slightly reflective ~ 1.0 fully reflect sky 
#define NL_GROUND_RAIN_WETNESS 1.0 // 0.0 no wetness ~ 1.0 fully wet blocks when raining
#define NL_GROUND_RAIN_PUDDLES 1.0 // 0.0 no puddles ~ 1.0 puddles
//#define NL_GROUND_AURORA_REFL    // [toggle] aurora reflection on ground

/* Entity (Actor, ItemInHand) */
#define NL_ENTITY_BRIGHTNESS     0.6 // 0.1 dark ~ 1.6 bright
#define NL_ENTITY_EDGE_HIGHLIGHT 0.45 // 0.0 no highlight ~ 1.6 bright highlight

/* Weather particles */
#define NL_WEATHER_SPECK 1.0         // [toggle] 0.0 vanilla texture ~ 1.0 soft speck
#define NL_WEATHER_RAIN_SLANT 3.5    // 1.0 minimal ~ 8.0 violent
#define NL_WEATHER_PARTICLE_SIZE 0.9 // 0.5 tiny ~ 4.0 large

/* Lava effects */
#define NL_LAVA_NOISE            // [toggle] darken lava in certain regions
#define NL_LAVA_NOISE_BUMP 0.32 // [toggle] 0.1 subtle ~ 0.8 massive waves
#define NL_LAVA_NOISE_SPEED 0.02 // 0.0 still ~ 0.1 fast

/* Black Hole */
#define NL_BH_COL_LOW vec3(0.1, 0.1, 1.0)
#define NL_BH_COL_HIGH vec3(2.8, 0.1, 0.6)
#define NL_BH_DIST 1.8
#define NL_BH_SPEED 0.3

/*
  Subpack names and flags are inside `pack_config.toml`.
  Build tool will enable corresponding flags when compiling. 
*/

#ifdef NO_WAVE_NO_FOG
  #define NO_WAVE
  #define NO_FOG
#endif


#ifdef NO_FOG
  #undef NL_FOG
#endif


#ifdef NO_WAVE
  #undef NL_PLANTS_WAVE
  #undef NL_LANTERN_WAVE
  #undef NL_UNDERWATER_WAVE
  #undef NL_WATER_WAVE
  #undef NL_RAIN_MIST_OPACITY
#endif 

#ifdef NO_CHUNK_ANIM
  #undef NL_CHUNK_LOAD_ANIM
#endif

#ifdef ROUNDED_CLOUDS
  #undef NL_CLOUD_TYPE
  #define NL_CLOUD_TYPE 2
#endif

#ifdef NO_SHOOTING_STARS
  #undef falling_stars
#endif

#ifdef NO_AURORA
  #undef NL_AURORA
  #undef NL_AURORA_VELOCITY
  #undef NL_AURORA_SCALE
  #undef NL_AURORA_WIDTH
  #undef NL_AURORA_COL1
  #undef NL_AURORA_COL2
  #undef LYNX_AURORA
#endif

#ifdef NEWB_AURORA
  #undef LYNX_AURORA
  #define NL_AURORA
#endif

#endif

/*
materials = ["Actor", "Sky", "RenderChunk", "EndSky", "Clouds", "SunMoon", "Weather", "ItemInHandTextured", "ItemInHandColor", "Stars"]
[[subpack]]
define = "NO_WAVE_NO_FOG"
materials = ["RenderChunk"]
description = '''
Removes both fog and wave
'''

[[subpack]]
define = "NO_FOG"
materials = ["RenderChunk"]
description = '''
Removes fog
'''

[[subpack]]
define = "NO_WAVE"
materials = ["RenderChunk"]
description = '''
Removes wave
'''

[[subpack]]
define = "NO_SHOOTING_STARS"
materials = ["Sky"]
description = '''
Removes shooting stars 
'''

[[subpack]]
define = "ROUNDED_CLOUDS"
materials = ["Clouds"]
description = '''
Rounded Complementary style clouds
'''

[[subpack]]
define = "NO_AURORA"
materials = ["Sky"]
description = '''
Removes Aurora
'''
*/
